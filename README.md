...
**Функция входа есть.** На стартовом экране вводится номер телефона, страна
выбирается клавишами влево-вправо, «Далее» поднимает точку доступа и отправляет
первый пакет MTProto (`req_pq_multi`) в дата-центр Telegram. Запрос кода по SMS
ещё впереди: для этого нужны RSA, DH и `api_id` с my.telegram.org.

| Что показывает приложение сейчас | Как список выглядит при заполнении |
| :---: | :---: |
| <img src="docs/images/sign-in.png" width="220" alt="Ввод номера телефона"> | <img src="docs/images/chat-list.png" width="220" alt="Макет оформления списка чатов"> |
...
| 2. Прототип: запуск, авторизация, список чатов, текст | ввод номера и первый пакет MTProto; кода и сессии ещё нет |
