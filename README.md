...
Работает оболочка приложения: экран списка чатов в фирменном оформлении, иконка,
локализация, сборка под устройство и упаковка в самоподписанный `.sisx`.

**Сетевого слоя пока нет.** Список чатов заполняется демонстрационными данными из
ресурсов — подключения к Telegram, авторизации и сообщений в приложении ещё не существует.

<img src="docs/images/chat-list.png" width="240" alt="Список чатов">

Изображение выше — не снимок с устройства, а отрисовка макета скриптом
`tools\preview-ui.ps1` по тем же правилам раскладки, что зашиты в код: эмулятор
на современных Windows не запускается, о чём ниже.
...
| 2. Прототип: запуск, авторизация, список чатов, текст | интерфейс списка готов, протокол не начат |
...
tools\preview-ui.ps1         REM отрисовывает макет интерфейса
```
