...
На телефоне: **Настройки → Обновление** (или «Опции → Обновление»). GitHub
с N95 по HTTPS недоступен (нужен TLS 1.2), поэтому `.sisx` скачивается на
компьютер в `C:\Data\Symgram\` или в корень карты как `Symgram-0.2.18.sisx`.
Приложение находит файл и открывает системный установщик. Ставить поверх,
не удаляя предыдущую версию.
